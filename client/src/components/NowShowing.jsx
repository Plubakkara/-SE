...
const NowShowing = ({ movies, selectedMovieIndex, setSelectedMovieIndex, auth, isFetchingMoviesDone }) => {
	return (
		<div style={{ backgroundColor: '#C6EBC5' }} className="mx-4 flex flex-col rounded-md p-4 text-gray-900 drop-shadow-md sm:mx-8 sm:p-6">
							...
							{movies?.map((movie, index) => {
								return movies[selectedMovieIndex]?._id === movie._id ? (
									<div
										key={index}
										title={movie.name}
										className="flex w-[108px] flex-col rounded-md bg-gradient-to-br from-green-400 to-red-400 p-1 text-white drop-shadow-md hover:from-red-400 hover:to-red-400 sm:w-[144px]"
										onClick={() => {
									...
									<div
										key={index}
										className="flex w-[108px] flex-col rounded-md bg-white p-1 drop-shadow-md hover:bg-gradient-to-br hover:from-red-400 hover:to-red-400 hover:text-white sm:w-[144px]"
										onClick={() => {
