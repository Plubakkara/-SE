...
const TheaterListsByCinema = ({ cinemas, selectedCinemaIndex, setSelectedCinemaIndex, fetchCinemas, auth }) => {
	...
	return (
		<div className="mx-4 h-fit rounded-md bg-primary-200 text-gray-900 drop-shadow-md sm:mx-8">
			<div className="flex items-center justify-center gap-2 rounded-t-md bg-primary-300 px-2 py-1.5 text-center text-2xl font-semibold text-white sm:py-2">
				{isEditing ? (
					<input
						title="Cinema name"
						type="text"
				...
				{auth.role === 'admin' && (
					<>
						{isEditing ? (
							<form onClick={handleSubmitName(onEditCinema)}>
								<button
									...
									className="flex w-fit items-center gap-1 rounded-md bg-gradient-to-r from-red-600 to-blue-500  py-1 pl-2 pr-1.5 text-sm font-medium text-white hover:from-indigo-500 hover:to-blue-400"
									onClick={() => {
										SetIsEditing(false)
									}}
								>
							...
							<button
								...
								className="flex w-fit items-center gap-1 rounded-md bg-primary-500  py-1 pl-2 pr-1.5 text-sm font-medium text-white hover:from-indigo-500 hover:to-blue-400"
						...
						<button
							title="Delete cinema"
							disabled={isDeleting}
							className="flex w-fit items-center gap-1 rounded-md bg-primary-50 py-1 pl-2 pr-1.5 text-sm font-medium text-white hover:from-red-600 hover:to-rose-600 disabled:from-slate-500 disabled:to-slate-400"
			...
			<div className="flex flex-col gap-6 p-4 sm:p-6 overflow-y-auto">
				...
				<form className="flex flex-col gap-4" onSubmit={handleSubmit(onIncreaseTheater)}>
					...
					{auth.role === 'admin' && (
						<div className="flex w-full flex-wrap justify-between gap-4 rounded-md bg-primary-300 to-white p-4">
							...
							<div className="flex grow flex-col gap-4 sm:justify-end md:flex-row">
								...
								<div className="flex grow md:grow-0">
									<div className="flex flex-col items-center justify-center gap-1 rounded-l bg-primary-600 p-1 text-black">
									...
									<button
										...
										className="flex grow items-center justify-center whitespace-nowrap rounded-r bg-primary-500 px-2 py-1 font-medium text-white drop-shadow-md hover:from-indigo-500 hover:to-blue-400 disabled:from-slate-500 disabled:to-slate-400 md:grow-0"
										type="submit"
									>
				...
				{auth.role === 'admin' && cinemas[selectedCinemaIndex].theaters.length > 0 && (
					<div className="flex justify-center">
						<button
							...
							className="w-fit rounded-md bg-primary-50 px-2 py-1 font-medium text-white drop-shadow-md hover:from-red-600 hover:to-rose-500 disabled:from-slate-500 disabled:to-slate-400"
