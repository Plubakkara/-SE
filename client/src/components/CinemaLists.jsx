...
const CinemaLists = ({
			...
			cinemasList.map((cinema, index) => {
				return cinemas[selectedCinemaIndex]?._id === cinema._id ? (
					<button
						...
						className="w-fit rounded-md px-2.5 py-1.5 text-lg font-medium text-white drop-shadow-xl"
						onClick={() => {
					...
					<button
						...
						className="w-fit rounded-md px-2 py-1 font-medium text-white drop-shadow-md hover:from-indigo-700 hover:to-blue-600"
						onClick={() => {
	...
	return (
		<>
			<div style={{ backgroundColor: '#C6EBC5' }} className="mx-4 flex h-fit flex-col gap-4 rounded-md p-4 text-gray-900 drop-shadow-xl sm:mx-8 sm:p-6">
				<form
				...
				>
					...
					{auth.role === 'admin' && (
						<div className="flex w-fit grow sm:justify-end">
							...
							<button
								disabled={isAdding}
								className="flex items-center whitespace-nowrap rounded-r-md bg-primary-500 px-2 py-1 font-medium text-white hover:from-indigo-500 hover:to-blue-400 disabled:from-slate-500 disabled:to-slate-400"
							>
