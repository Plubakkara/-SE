...
const TheaterShort = ({ theaterId, movies, selectedDate, filterMovie, rounded = false }) => {
	...
	return (
		<div
			className={`flex flex-col bg-gradient-to-br from-primary-400 to-white sm:flex-row sm:rounded-tr-none ${
				rounded && 'rounded-b-md'
			}`}
		>
			<div className="flex flex-col sm:flex-row">
				<div
					className={`flex min-w-[120px] flex-row items-center justify-center gap-x-2 bg-primary-500 px-4 py-0.5 text-2xl font-bold text-white sm:flex-col ${
				...
				{auth.role === 'admin' && (
					<div
						className={`flex w-full min-w-[160px] flex-row justify-center gap-x-4 border-b-2 border-indigo-200 bg-primary-400 px-4 py-0.5 text-sm font-bold sm:w-fit sm:flex-col sm:border-none`}
					>
